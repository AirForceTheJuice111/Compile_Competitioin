...
post-inline SCCP cleanup, whole-program unreachable-function removal, stable
trace block layout, safe AArch64 peephole rules, compact spill-frame allocation,
direct use of resident integer/pointer registers, direct scalar-float `s`
register transfers, immediate-aware arithmetic/comparisons, constant-multiply
strength reduction, and folded pointer-address generation. The existing
...
Imported FuncSpec, MemOpt, LoopSimplify, and LoopUnroll remain disabled
unless named explicitly in `SYSY_EXPERIMENTAL_PASSES` as `funcspec`, `memopt`,
`loopsimplify`, or `loopunroll`. Their general correctness or profitability is
not yet strong enough for the default pipeline. Trace block layout,
whole-program function DCE, and the safe subset of the textual AArch64
peephole are now stable defaults in optimized modes. The historically mixed
branch-to-next-label rewrite remains gated behind
`SYSY_AGGRESSIVE_PEEPHOLE=1`.
