...
fmjcc="${FMJCC:-$root/build/fmjcc}"
fmjinterp="${FMJINTERP:-$root/build/fmjinterp}"
...
collect_fuzz_cases() {
    for dir in "$root/test/all" "$root/test/submit"; do
        [[ -d "$dir" ]] || continue
        find "$dir" -type f -name '*.fmj' -print
    done | sort | while IFS= read -r src; do
