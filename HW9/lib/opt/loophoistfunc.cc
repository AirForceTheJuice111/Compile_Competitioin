...
bool operandsAreInvariant(QuadStm *stm, const set<int> &loopDefs, const set<int> &invariantDefs) {
...
QuadBlock *findPreheader(QuadFuncDecl *func, const set<int> &bodyBlocks, int headerLabel) {
...
vector<LoopHeader*> sortedLoops(LoopHeaderMap *loopHeaderMap, QuadFuncDecl *func) {
...
bool hoistOneLoop(QuadFuncDecl *func, LoopHeader *loop) {
    ...
    if (preheader == nullptr || preheader->quadlist == nullptr) return false;
    ...
    if (insertPos != preheader->quadlist->begin()) {
        auto last = insertPos;
        --last;
