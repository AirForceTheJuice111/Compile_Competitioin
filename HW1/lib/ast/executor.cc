...
void Executor::visit(BinaryOp *node) {
  ...
  if (opStr == "+")
    ...
    if (rval == 0) {
      cerr << "Error: Division by zero at line " << node->getPos()->sline
           << ", column " << node->getPos()->scolumn << endl;
      result = 0;
    } else {
      result = lval / rval;
    }
