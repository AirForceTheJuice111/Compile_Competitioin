...
VarDecl *VarDecl::clone() {
    ...
    if (holds_alternative<IntExp *>(init)) return new VarDecl(pos->clone(), type->clone(), id_clone, get<IntExp *>(init)->clone());
    else if (holds_alternative<vector<IntExp *> *>(init)) return new VarDecl(pos->clone(), type->clone(), id_clone, cloneList<IntExp>(get<vector<IntExp *> *>(init)));
    else return new VarDecl(pos->clone(), type->clone(), id_clone);
}
