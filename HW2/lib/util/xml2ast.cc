...
template <class T> // only works for lists of VarDecl, MethodDecl, Formal,
  ...
  variant<monostate, VarDecl *, MethodDecl *, Formal *, ClassDecl *, IntExp *>
      t = monostate{};
...
Type *create_type(XMLElement *element) {
  ...
  // get the typeKind
  IdExp *tk =
      create_leafnode<IdExp>(element, "Type", "typeKind", ATTR_TYPE::ID);
  if (tk->id == "CLASS")
  ...
  case TypeKind::INT: // look for int init
    while (ce1) {
      if (string(ce1->Name()) == "IntInit") {
        IntExp *ii =
            create_leafnode<IntExp>(ce1, "IntInit", "val", ATTR_TYPE::INT);
        init = ii;
      }
...
If *create_if(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (s != nullptr) {
      if (stm1 == nullptr)
        stm1 = s;
      else if (stm2 == nullptr)
        stm2 = s;
      else
        cerr << "Error: If: More than two statements in If" << endl;
    } else {
      Exp *e = create_exp(ce);
      if (e != nullptr)
        exp = e;
      else
        cerr << "Error: If: Unknown element in If" << endl;
    }
    ce = ce->NextSiblingElement();
  }
  ...
  while (ce) {
    ...
    if (s != nullptr) {
      if (stm == nullptr)
        ...
        cerr << "Error: While: More than one statement in While" << endl;
    } else {
      Exp *e = create_exp(ce);
      if (e != nullptr)
        exp = e;
      else
        cerr << "Error: While: Unknown element in While" << endl;
    }
    ce = ce->NextSiblingElement();
  }
...
PutInt *create_putInt(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (e != nullptr)
      ...
      cerr << "Error: PutInt: unknown lement in PutInt" << endl;
    ce = ce->NextSiblingElement();
  }
...
PutCh *create_putCh(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (exp == nullptr)
      ...
      cerr << "Error: PutCh: More than one expression in PutCh" << endl;
    ce = ce->NextSiblingElement();
  }
...
PutArray *create_putArray(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (exp1 == nullptr)
      ...
      cerr << "Error: PutArray: More than two expressions in PutArray" << endl;
    ce = ce->NextSiblingElement();
  }
...
CallExp *create_callExp(XMLElement *element) {
  ...
  while (ce) {
    if (string(ce->Name()) == "ParList") {
      ...
      Exp *e = create_exp(ce);
      if (exp == nullptr)
        exp = e; // first expression must be the object expression
      else if (id == nullptr &&
               e->getASTKind() ==
                   ASTKind::IdExp) // only if the ID is not the object, then
                                   // it's the method ID
        id = static_cast<IdExp *>(e);
      else {
...
Length *create_length(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (exp == nullptr)
      ...
      cerr << "Error: Length: More than one expression in Length" << endl;
    ce = ce->NextSiblingElement();
  }
...
NewArray *create_newArray(XMLElement *element) {
  ...
  while (ce) {
    ...
    if (size == nullptr)
      ...
      cerr << "Error: NewArray: More than one expression in NewArray" << endl;
    ce = ce->NextSiblingElement();
  }
  if (size == nullptr) {
  ...
  while (ce) {
    ...
    if (e != nullptr) {
      if (exp == nullptr)
        ...
        cerr << "Error: GetArray: More than one expression in GetArray" << endl;
    } else {
