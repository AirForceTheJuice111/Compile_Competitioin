...
private:
  Kind s_kind;
  TypeKind
      typeKind; // enum class TypeKind {CLASS/OBJECT = 0, INT = 1, ARRAY = 2};
  variant<monostate, string, int>
      type_par; // string for class name, int for array arity
...
private:
  AST_Semant_Map *semant_map; // this is the semantic information map for all
                              // (sub)expressions
  ...
  int in_a_while_loop = 0; // for break and continue (handles nested while
                           // loops)
public:
  int mode;
