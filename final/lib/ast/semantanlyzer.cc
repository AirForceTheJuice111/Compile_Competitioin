...
void AST_Semant_Visitor::visit(MethodDecl *node) {
    ...
    if (node->type != nullptr && node->type->typeKind == TypeKind::CLASS) {
        if (node->type->cid == nullptr || !name_maps->is_class(node->type->cid->id)) {
            cerr << "Error: at position " << node->get_pos()->to_str() << endl;
            cerr << "Error: Unknown class return type: "
                 << (node->type->cid == nullptr ? "<null>" : node->type->cid->id) << endl;
            exit(1);
        }
    }
    if (node->fl != nullptr)
        for (auto f : *(node->fl)) f->accept(*this);
    if (node->vdl != nullptr)
        for (auto vd : *(node->vdl)) vd->accept(*this);
    if (node->sl != nullptr)
        for (auto s : *(node->sl)) s->accept(*this);
...
void AST_Semant_Visitor::visit(Formal *node) {
    ...
    if (node->type->typeKind == TypeKind::CLASS) {
        if (node->type->cid == nullptr || !name_maps->is_class(node->type->cid->id)) {
            cerr << "Error: at position " << node->get_pos()->to_str() << endl;
            cerr << "Error: Unknown class type in formal: "
                 << (node->type->cid == nullptr ? "<null>" : node->type->cid->id) << endl;
            exit(1);
        }
    }
...
void AST_Semant_Visitor::visit(Return *node) {
    ...
    Formal *ret_formal = name_maps->get_method_formal(
        current_visiting_class,
        current_visiting_method,
        "_^return^_" + current_visiting_method
    );
...
void AST_Semant_Visitor::visit(This *node) { // This 就是 this 关键字，代表当前对象。需要检查 this 是否在类方法中使用，以及设置语义信息：this 的类型是当前类，是一个值（Value），但不是 lvalue（因为不能赋值）。另外，this 只能在 __$main__ 或其他非类方法中使用，需要报错。
    if (node == nullptr) return;
    // this 只能在类方法中使用（不能在 __$main__ 中）
    if (current_visiting_class == "__$main__" || current_visiting_class.empty()) {
        cerr << "Error: at position " << node->get_pos()->to_str() << endl;
...
void AST_Semant_Visitor::visit(GetArray *node) {
    ...
    if (!sem->is_lvalue()) {
        cerr << "Error: at position " << node->get_pos()->to_str() << endl;
        cerr << "Error: getarray() argument must be an array lvalue" << endl;
        exit(1);
    }
...
void AST_Semant_Visitor::visit(IdExp *node) { // IdExp 代表标识符表达式，如 foo、bar 等。需要按照名称查找优先顺序（方法局部变量 -> 方法形参 -> 类变量）查找标识符，并设置语义信息：标识符的类型和是否是 lvalue。同时，如果标识符未找到，需要报错。
    ...
    // 3. 裸类变量访问不合法，必须通过 this.x 或 obj.x。
    if (current_visiting_class != "__$main__") {
        vd = name_maps->get_class_var(current_visiting_class, id);
        if (vd == nullptr) {
            string parent = name_maps->get_parent(current_visiting_class);
            if (!parent.empty()) vd = name_maps->get_class_var(parent, id);
        }
        if (vd != nullptr) {
            cerr << "Error: at position " << node->get_pos()->to_str() << endl;
            cerr << "Error: Class variable " << id << " must be accessed via object (this."
                 << id << " or obj." << id << ")" << endl;
            exit(1);
        }
    }
