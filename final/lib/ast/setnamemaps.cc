 ...
 *   - MainMethod 被视为 __$main__ 类中的 main 方法
...
void AST_Name_Map_Visitor::visit(MainMethod *node) {
    ...
    string retName = "_^return^_main";
    IdExp *retId = new IdExp(pos->clone(), retName);
    Formal *retFormal = new Formal(pos->clone(), retType, retId);
    name_maps->add_method_formal("__$main__", "main", retName, retFormal);
    name_maps->add_method_formal_list("__$main__", "main", {retName});
...
void AST_Name_Map_Visitor::visit(MethodDecl *node) {
    ...
    Pos *pos = node->get_pos()->clone();
    string retName = "_^return^_" + method_name;
    IdExp *retId = new IdExp(pos, retName);
    Formal *retFormal = new Formal(pos->clone(), retType, retId);
    name_maps->add_method_formal(current_visiting_class, method_name, retName, retFormal);
    formal_names.push_back(retName);
