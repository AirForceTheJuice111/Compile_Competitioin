...
Program *xml2ast(XMLElement *element, AST_Semant_Map **sm) {
    ...
    if (element == nullptr) {
        cerr << "Error: null XML root" << endl;
        return nullptr;
    }

    XMLElement *program = nullptr;
    string root_name = element->Name();
    if (root_name == "Program") {
        program = element;
    } else if (root_name == "FDMJAST") {
        XMLElement *first = element->FirstChildElement();
        if (first != nullptr && string(first->Name()) == "NameMaps") {
            global_semant_map->setNameMaps(create_NameMaps(first));
            program = first->NextSiblingElement();
        } else {
            program = first;
        }
    } else {
        cerr << "Error: first element is neither FDMJAST nor Program" << endl;
        return nullptr;
    }

    if (program == nullptr || string(program->Name()) != "Program") {
...
template <class T>
T *create_leafnode(XMLElement *, string, string, ATTR_TYPE); // forward declaration
