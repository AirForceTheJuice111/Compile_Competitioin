...
static bool stmMayFallThrough(tree::Stm *stm) {
    if (stm == nullptr) return true;

    switch (stm->getTreeKind()) {
    case tree::Kind::JUMP:
    case tree::Kind::RETURN:
        return false;
    case tree::Kind::SEQ: {
        auto seq = static_cast<tree::Seq *>(stm);
        if (seq->sl == nullptr || seq->sl->empty()) return true;

        bool reachable = true;
        for (auto child : *seq->sl) {
            if (child == nullptr) continue;
            if (!reachable && child->getTreeKind() != tree::Kind::LABELSTM) {
                continue;
            }
            reachable = stmMayFallThrough(child);
        }
        return reachable;
    }
    default:
...
static bool appendTranslatedStm(vector<tree::Stm *> *sl, Tr_Exp *tr, Temp_map *tm) {
    if (sl == nullptr || tr == nullptr) return true;
    auto nx = tr->unNx(tm);
    if (nx == nullptr || nx->stm == nullptr) return true;
    sl->push_back(nx->stm);
    return stmMayFallThrough(nx->stm);
}
...
static tree::Exp *tempPtrExp(tree::Temp *temp) {
...
static tree::Stm *storeObjectField(tree::Temp *obj_temp, int offset, tree::Type field_type, tree::Exp *value) {
    return new tree::Move(new tree::Mem(field_type, fieldAddrExp(obj_temp, offset)), value);
}

static void appendArrayLiteralAllocation(vector<tree::Stm *> *sl, Temp_map *tm, tree::Temp *dst_temp, vector<fdmj::IntExp *> *init_arr) {
    if (sl == nullptr || tm == nullptr || dst_temp == nullptr || init_arr == nullptr) return;
    ...
    sl->push_back(new tree::Move(
        new tree::Mem(tree::Type::INT, tempPtrExp(dst_temp)),
        new tree::Const(len)));
...
void ASTToTreeVisitor::visit(fdmj::MethodDecl *node) {
    ...
    // Build FuncDecl args: [this, explicit formals...], matching the call ABI.
    ...
    args->push_back(new tree::Temp(this_temp->num));
    if (node->fl != nullptr) {
        for (auto formal : *node->fl) {
            if (formal == nullptr || formal->id == nullptr) continue;
            auto formal_temp = method_var_table->get_var_temp(formal->id->id);
            if (formal_temp != nullptr) args->push_back(new tree::Temp(formal_temp->num));
        }
    }
...
void ASTToTreeVisitor::visit(fdmj::If *node) {
    ...
    bool then_falls = stmMayFallThrough(then_stm);
    bool else_falls = stmMayFallThrough(else_stm);
    bool needs_end = then_falls || else_falls;

    // Get condition as Tr_cx (already done above)
    // Allocate labels for if
    auto true_label = method_temp_map->newlabel();
    auto end_label = needs_end ? method_temp_map->newlabel() : nullptr;
    auto false_label = (else_stm == nullptr && end_label != nullptr)
        ? end_label
        : method_temp_map->newlabel();
    ...
    if (then_falls && end_label != nullptr) sl->push_back(new tree::Jump(end_label));
    if (false_label != end_label) sl->push_back(new tree::LabelStm(false_label));
    if (else_stm != nullptr) sl->push_back(else_stm);
    if (end_label != nullptr) sl->push_back(new tree::LabelStm(end_label));
...
void ASTToTreeVisitor::visit(fdmj::While *node) {
    ...
    if (stmMayFallThrough(body_stm)) sl->push_back(new tree::Jump(start_label));
...
void ASTToTreeVisitor::visit(fdmj::NewObject *node) {
    ...
    // Initialize fields declared by this class and its ancestors. Slots use the
    // ClassName^field key, so hidden fields and inherited fields both get their
    // own object storage.
    vector<string> class_chain;
    for (string cur = class_name; !cur.empty(); cur = nm->get_parent(cur))
        class_chain.push_back(cur);
    reverse(class_chain.begin(), class_chain.end());

    for (auto &decl_class : class_chain) {
        auto vars = nm->get_class_var_list(decl_class);
        if (vars == nullptr) continue;
        for (auto &field_name : *vars) {
            auto field_decl = nm->get_class_var(decl_class, field_name);
            if (field_decl == nullptr) continue;

            int offset = class_table->get_var_pos(decl_class, field_name);
            tree::Type field_type = typeKind2TreeType(field_decl->type->typeKind);

            sl->push_back(storeObjectField(ptr_temp, offset, field_type, new tree::Const(0)));

            if (field_decl->type->typeKind == fdmj::TypeKind::INT &&
                holds_alternative<fdmj::IntExp *>(field_decl->init)) {
                auto init_int = get<fdmj::IntExp *>(field_decl->init);
                if (init_int != nullptr)
                    sl->push_back(storeObjectField(ptr_temp, offset, tree::Type::INT, new tree::Const(init_int->val)));
            } else if (field_decl->type->typeKind == fdmj::TypeKind::ARRAY &&
                       holds_alternative<vector<fdmj::IntExp *> *>(field_decl->init)) {
                auto init_arr = get<vector<fdmj::IntExp *> *>(field_decl->init);
                if (init_arr != nullptr) {
                    auto arr_temp = method_temp_map->newtemp();
                    appendArrayLiteralAllocation(sl, method_temp_map, arr_temp, init_arr);
                    sl->push_back(storeObjectField(ptr_temp, offset, tree::Type::PTR, tempPtrExp(arr_temp)));
                }
            }
        }
        delete vars;
