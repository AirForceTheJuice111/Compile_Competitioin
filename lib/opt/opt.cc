...
static set<Temp*> *emptyTempSet() {
    return new set<Temp*>();
}

static set<Temp*> *oneTempSet(int temp_num) {
    auto *out = new set<Temp*>();
    out->insert(new Temp(temp_num));
    return out;
}

static void addTermUses(set<Temp*> *uses, QuadTerm *term) {
    if (uses == nullptr || term == nullptr || term->kind != QuadTermKind::TEMP) return;
    auto *temp = term->get_temp();
    if (temp != nullptr && temp->temp != nullptr) uses->insert(new Temp(temp->temp->num));
}

static QuadMove *makeMove(QuadTemp *dst, QuadTerm *src) {
    int dst_num = (dst == nullptr || dst->temp == nullptr) ? -1 : dst->temp->num;
    auto *def = dst_num >= 0 ? oneTempSet(dst_num) : emptyTempSet();
    auto *use = emptyTempSet();
    addTermUses(use, src);
    return new QuadMove(dst, src, def, use);
}

static QuadMove *makeConstMove(int dst_num, int value, QuadType type) {
    return makeMove(new QuadTemp(new Temp(dst_num), type), new QuadTerm(value));
}

static QuadJump *makeJump(Label *label) {
    return new QuadJump(label, emptyTempSet(), emptyTempSet());
}

static bool isExitCallStmt(QuadStm *stm) {
    if (stm == nullptr) return false;
    if (stm->kind == QuadKind::EXTCALL) {
        auto *ext = static_cast<QuadExtCall*>(stm);
        return ext != nullptr && ext->extfun == "exit";
    }
    if (stm->kind == QuadKind::MOVE_EXTCALL) {
        auto *move_ext = static_cast<QuadMoveExtCall*>(stm);
        return move_ext != nullptr && move_ext->extcall != nullptr && move_ext->extcall->extfun == "exit";
    }
    ...
    if (stm->kind == QuadKind::MOVE_CALL) {
        ...
        return move_call != nullptr && move_call->call != nullptr && move_call->call->name == "exit";
    }
    return false;
}

static bool isExplicitTerminator(QuadStm *stm) {
    if (stm == nullptr) return false;
...
static bool blockHasExplicitTerminator(QuadBlock *block) {
    if (block == nullptr || block->quadlist == nullptr) return false;
    for (auto it = block->quadlist->rbegin(); it != block->quadlist->rend(); ++it) {
        if (*it == nullptr) continue;
        return isExplicitTerminator(*it);
    }
    return false;
}

static bool blockHasExitLabel(QuadBlock *block, int label_num) {
    if (block == nullptr || block->exit_labels == nullptr) return false;
    for (auto *label : *block->exit_labels) {
        if (label != nullptr && label->num == label_num) return true;
    }
    return false;
}
...
static bool isEdgeExecutable(Opt *opt, int pred, int curr) {
    ...
    for (auto *s : *it->second->quadlist) {
        ...
        if (s->kind == QuadKind::CJUMP) {
            ...
            return (cj->t != nullptr && cj->t->num == curr) ||
                   (cj->f != nullptr && cj->f->num == curr); // condition unknown: both CFG branches are potentially executable
        }
        if (isExitCallStmt(s) || s->kind == QuadKind::RETURN) return false;
    }
    return blockHasExitLabel(it->second, curr);
}
    ...
    while (changed) {
        ...
        for (auto *block : *func->quadblocklist) {
            ...
            for (auto *stm : *block->quadlist) {
                ...
                has_explicit_terminator |= isExplicitTerminator(stm);
                switch (stm->kind) {
                    case QuadKind::MOVE: {
            ...
            if (!has_explicit_terminator && block->exit_labels != nullptr) {
                for (auto *label : *block->exit_labels) {
                    if (label != nullptr)
                        changed |= updateExecutable(block_executable, label->num);
                }
            }
        }
    }
...
void Opt::modifyFunc() {
    ...
    for (auto *block : *func->quadblocklist) {
            ...
            switch (stm->kind) {
                ...
                case QuadKind::JUMP: {
                    ...
                    for (auto &[fn, fv, ft] : fmoves)
                        new_quadlist->push_back(makeConstMove(fn, fv, ft));
                ...
                case QuadKind::CJUMP: {
                    ...
                    if (t_exec && !f_exec) {
                        ...
                        new_quadlist->push_back(makeJump(new Label(cjump->t->num)));
                    } else if (!t_exec && f_exec) {
                        new_exits->push_back(new Label(cjump->f->num));
                        new_quadlist->push_back(makeJump(new Label(cjump->f->num)));
                ...
                case QuadKind::PHI: {
                    ...
                    if (new_args->size() == 1) {
                        ...
                        new_quadlist->push_back(makeMove(phi->temp_exp->clone(), src));
                    } else {
        ...
        if (!blockHasExplicitTerminator(block)) {
            vector<Label*> executable_exits;
            if (block->exit_labels != nullptr) {
                for (auto *label : *block->exit_labels) {
                    if (label != nullptr && isEdgeExecutable(this, curr, label->num))
                        executable_exits.push_back(label);
                }
            }
            if (!executable_exits.empty()) {
                for (auto &[fn, fv, ft] : fmoves)
                    new_quadlist->push_back(makeConstMove(fn, fv, ft));
                for (auto *label : executable_exits)
                    new_exits->push_back(new Label(label->num));
                if (executable_exits.size() == 1)
                    new_quadlist->push_back(makeJump(new Label(executable_exits.front()->num)));
            }
        }
