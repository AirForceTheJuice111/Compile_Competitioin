...
set<string> *Name_Maps::get_method_list(string class_name) {
    ...
    for (auto &method : methods) {
        if (method.first == class_name) out->insert(method.second);
    }
    return out;
}
...
set<string> *Name_Maps::get_class_var_list(string class_name) {
    set<string> *out = new set<string>();
    for (auto &var : classVar) {
        if (var.first.first == class_name) out->insert(var.first.second);
    }
    return out;
}
...
set<string> *Name_Maps::get_method_var_list(string class_name, string method_name) {
    set<string> *out = new set<string>();
    for (auto &var : methodVar) {
        if (get<0>(var.first) == class_name && get<1>(var.first) == method_name) {
            out->insert(get<2>(var.first));
        }
    }
    return out;
}
...
vector<Formal*>* Name_Maps::get_method_formal_list(string class_name, string method_name) {
    ...
    if (methodFormalList.find(p) == methodFormalList.end()) {
        ...
        if (!parent.empty() && methodFormalList.find(pair<string, string>(parent, method_name)) != methodFormalList.end()) {
            auto &vl = methodFormalList[pair<string, string>(parent, method_name)];
            for (auto v : vl) fl->push_back(Name_Maps::get_method_formal(parent, method_name, v));
            return fl;
        }
        return fl;
    }
...
vector<string> *Name_Maps::get_method_formal_list_string(string class_name, string method_name) {
    ...
    if (methodFormalList.find(p) == methodFormalList.end()) {
        string parent = get_parent(class_name);
        if (!parent.empty() && methodFormalList.find(pair<string, string>(parent, method_name)) != methodFormalList.end()) {
            for (auto &v : methodFormalList[pair<string, string>(parent, method_name)]) out->push_back(v);
        }
        return out;
    }
    for (auto &v : methodFormalList[p]) out->push_back(v);
    return out;
}
